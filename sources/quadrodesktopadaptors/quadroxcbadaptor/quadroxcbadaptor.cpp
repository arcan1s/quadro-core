...
QHash<long long, unsigned long long> QuadroXCBAdaptor::getWindowsList()
{
    ...
    for (unsigned int i = 0; i < clientListSize / sizeof(Window); i++) {
        long long *pid = reinterpret_cast<long long *>(
            getPropery(clientList[i], XA_CARDINAL, "_NET_WM_PID", nullptr));
                                   ...
                                   unsigned long *_size) const
{
    ...
    if (XGetWindowProperty(QX11Info::display(), _win, xaPropName, 0,
                           MAX_PROPERTY_VALUE_LEN / 4, false, _xaPropType,
                           &xaRetType, &retFormat, &retNItems, &retBytesAfter,
                           &retProperty)
        != Success) {
